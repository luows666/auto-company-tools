...
ENGINE="${ENGINE:-codex}"  # Set to "claude" to use Claude Code CLI
CODEX_BIN="${CODEX_BIN:-}"
CLAUDE_BIN="${CLAUDE_BIN:-}"  # Optional: specify Claude binary path
...
resolve_claude_bin() {
    ...
    # Prefer WSL-local Claude installed via nvm.
    local nvm_candidate=""
...
run_claude_cycle() {
    ...
    (
        ...
        echo "$prompt" | "$RESOLVED_CLAUDE_BIN" --print 2>&1
    ) > "$output_file" 2>&1 &
    local claude_pid=$!
    ...
    OUTPUT=$(cat "$output_file")
    RESULT_MESSAGE=""  # Claude doesn't have message file output like Codex
    rm -f "$output_file"
...
if [ "$ENGINE" = "claude" ]; then
    log "Engine: claude | Model: $MODEL_LABEL"
    log "Claude bin: $RESOLVED_CLAUDE_BIN"
    claude_version=$("$RESOLVED_CLAUDE_BIN" --version 2>/dev/null | head -n1 || true)
